...
export function AgentChatHistory({ logs, logsStatus, isThinking }: AgentChatHistoryProps) {
	...
	return (
		<div className="agent-chat-history">
			...
			<div className="chat-history-content" ref={containerRef} onScroll={handleScroll}>
				{logs.length === 0 ? (
					<div className={`chat-history-empty ${logsStatus === "error" ? "chat-history-error" : ""}`}>
						{emptyMessage}
					</div>
				) : (
					logs.map((log) => (
