...
function isIpcCommand(msg: unknown): msg is { method: string; params?: unknown } {
	return (
		typeof msg === "object" &&
		msg !== null &&
		!("requestId" in msg) &&
...
export class TasksPanel implements vscode.WebviewViewProvider, vscode.Disposable {
	...
	private async handleCommand(message: { method: string; params?: unknown }): Promise<void> {
	...
	private async fetchTaskLogs(
		user: string,
		taskId: string,
	): Promise<{ logs: TaskLogEntry[]; status: "ok" | "not_available" | "error" }> {
		try {
